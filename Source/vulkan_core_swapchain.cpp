...
namespace Vulkan {
  ...
  void SwapChain::createSwapChain() {
    ...
    Device::QueueFamilyIndices indices = pDevice->findQueueFamilies(pDevice->physicalDevice);
    ...
    if (vkCreateSwapchainKHR(pDevice->device, &createInfo, nullptr, &swapchain) != VK_SUCCESS) { 
      throw std::runtime_error("failed to create swap chain");
    }

    vkGetSwapchainImagesKHR(pDevice->device, swapchain, &imageCount, nullptr);
    vkSwapChainImages.resize(imageCount);
    vkGetSwapchainImagesKHR(pDevice->device, swapchain, &imageCount, vkSwapChainImages.data());
  ...
  void SwapChain::createImageViews() {
    ...
    for (size_t i = 0; i < vkSwapChainImages.size(); i++) {
      ...
      if (vkCreateImageView(pDevice->device, &createInfo, nullptr, &vkSwapChainImageViews[i]) != VK_SUCCESS) {
